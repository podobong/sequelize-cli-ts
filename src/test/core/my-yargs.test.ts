    ...
    describe('Core-myyargs', () => {
        ...
        describe('addCommands', () => {
            it('Create multiple commands', () => {
                exec('node ../build/test/core/add-commands.test.js miku', (error, stdout, stderr) => {
                    assert(stdout.includes('miku: sing=false'))
                })
                exec('node ../build/test/core/add-commands.test.js miku -s', (error, stdout, stderr) => {
                    assert(stdout.includes('miku: sing=true'))
                })
                exec('node ../build/test/core/add-commands.test.js miku --sing', (error, stdout, stderr) => {
                    assert(stdout.includes('miku: sing=true'))
                })
